...
type windowsService struct {
	c             *cli.Context
	startCall     func(c *cli.Context) error
	stopCall      func(c *cli.Context) error
	errSync       sync.Mutex
	stopStartErr  error
	isInteractive bool
	signals       []os.Signal
...
func (ws *windowsService) Execute(args []string, r <-chan wsvc.ChangeRequest, changes chan<- wsvc.Status) (bool, uint32) {
	...
	if err := ws.startCall(ws.c); err != nil {
		ws.setError(err)
		return true, 1
	}
