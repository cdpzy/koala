	...
	for key, _ := range params.Values {
		...
		if _, ok := fieldValues[fieldName]; !ok {
			...
			if !fieldValue.IsValid() {
				log.Warn("bindStruct: Field not found:", fieldName)
				continue
			}
			if !fieldValue.CanSet() {
				log.Warn("bindStruct: Field not settable:", fieldName)
				continue
			}
...
func getMultipartFile(params *Params, name string) multipart.File {
	for _, fileHeader := range params.Files[name] {
		...
		log.Infoln("Failed to open uploaded file", name, ":", err)
	}
	return nil
}
...
func bindFile(params *Params, name string, typ reflect.Type) reflect.Value {
	...
	if err != nil {
		log.Infoln("Failed to create a temp file to store upload:", err)
		return reflect.Zero(typ)
	}
	...
	if err != nil {
		log.Infoln("Failed to copy upload to temp file:", err)
		return reflect.Zero(typ)
	}

	_, err = tmpFile.Seek(0, 0)
	if err != nil {
		log.Infoln("Failed to seek to beginning of temp file:", err)
		return reflect.Zero(typ)
	}
...
func bindByteArray(params *Params, name string, typ reflect.Type) reflect.Value {
	if reader := getMultipartFile(params, name); reader != nil {
		...
		log.Infoln("Error reading uploaded file contents:", err)
	}
	return reflect.Zero(typ)
}
...
func Unbind(output map[string]string, name string, val interface{}) {
	if binder, found := binderForType(reflect.TypeOf(val)); found {
		if binder.Unbind != nil {
			...
			log.Warnf("binder: can not unbind %s=%s", name, val)
		}
	}
}
...
func binderForType(typ reflect.Type) (Binder, bool) {
	...
	if !ok {
		...
		if !ok {
			log.Warnf("binder: no binder for type:", typ)
			return Binder{}, false
		}
	}
	return binder, true
